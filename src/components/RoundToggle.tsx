...
export const RoundToggle = ({ enabled = true, size = 20 }: RoundToggleProps) => {
  ...
  return (
    ...
    >
      <motion.svg
        style={{
          position: "absolute", width: handleWidth, height: handleWidth, left: togglePadding, top: togglePadding, filter: `drop-shadow(0 0 ${size / 10}px rgba(0, 0, 0, 0.3))`
        }}
        animate={{ x: isOn ? toggleWidth - handleWidth - togglePadding * 2 - 0.5 : 0.5, rotate: isOn ? 60 * rotations : 0, color: isOn ? onColor : offColor, transition }}
        initial={false}
        width={handleWidth}
        height={handleWidth}
        viewBox="0 0 12 12"
